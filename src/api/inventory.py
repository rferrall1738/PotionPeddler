...
def get_inventory():
    ...
    with db.engine.begin() as connection:
        result = connection.execute(sqlalchemy.text("SELECT num_green_potions, num_green_ml, num_red_potions, num_red_ml, num_blue_potions, num_blue_ml from global_inventory"))
        inventory = result.fetchone()
        num_green_potions = inventory[1]
        num_green_ml = inventory[2]
        num_red_potions = inventory[3]
        num_red_ml = inventory[4]
        num_blue_potions = inventory[5]
        num_blue_ml = inventory[6]
        gold = inventory[7]
